    ...
    if (!($_SERVER['REQUEST_METHOD'] === 'POST')) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($action === 'follow-user') {
                ...
                if(!($user_id == $follow_user_id)) {
                    ...
                    if ($query->rowCount()){
                        ...
                        if (!$query->rowCount()){    
                            $sql = "insert into users_followed values(null,:user_Id,:follow_user_Id,null)";
                            $query = $db->prepare( $sql );
            ...
            elseif ($action === 'newNotificationCount') {
                $sql = "select sum(newNotificationCount) newNotificationCount from (
                select count(*) as newNotificationCount from user_words uw 
                ...
                select count(*) as newNotificationCount from users_followed uf0 
                    inner join users_followed uf on uf0.user_Id = uf.followed_user_Id 
            		inner join users u on uf.user_Id = u.Id and uf0.row_created_date >= u.last_seen_date
            		where u.Id = :user_Id and uf0.followed_user_Id <> :user_Id
                
                union all
            		
                select count(*)
                    from users_followed uf  inner join users u on uf.followed_user_Id = u.Id where uf.followed_user_Id = :user_Id
                   	and uf.row_created_date >= u.last_seen_date) ntf_cnt";
                $query = $db->prepare( $sql );
