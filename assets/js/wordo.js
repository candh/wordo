...
jQuery(document).ready(function($){
	var initialpop = true;
	var typeList = ["Adjective","Noun","Verb","Adverb","Pronoun","Proper noun","Article","Determiner","Preposition","Interjection","Initialism"];
	...
	if(window.location.pathname.length > 1){
		if($('.search').length){
			...
			// executeSearch(unescape(window.location.pathname.substr(1)).replace(/_+/g,' '));
	...
	$('#foo').submit(function(e){
		...
		//window.location.href=$('#foo .search').val();
		//This is where we update the address bar with the 'url' parameter
		// .trim().replace(/ +/g,'-')
		window.history.pushState('', document.title, $('#foo .search').val().trim().replace(/ +/g,'_'));

		//This stops the browser from actually following the link
		e.preventDefault();
		executeSearch(($('#foo .search').val()).replace(/_+/g,' '));
		return false;
	});
	function checkFavoriteButton(word){
		...
		$.post(url,mydata, function(data){
				...
				if(data == 'favorite'){
					...
					//loadingImg.hide();
          $('.mywl-link .liked-heart').show();
          $('.mywl-link .unliked-heart').hide();
				} else {
					//$('.mywl-link').removeClass('favorited');
					//loadingImg.hide();
          $('.mywl-link .unliked-heart').show();
          $('.mywl-link .liked-heart').hide();
				}
		});
	}
	...
	function executeSearch(word){
			...
			document.title = word + ' definition by Wordo';
			//document.title = title + ' definition';
	...
	function resultSelect(x10){
      ...
      if(myResultsList.length == 1){
				...
				if(myres.match(/\[/) == null && myres.match(/\s/) == null ){
					window.history.pushState(myres, document.title, myres.trim().replace(/ +/g,'_'));
					//myconsole(myres);
	...
	$(window).bind('popstate', function(event) {
		if(!initialpop){
			if(window.location.pathname.length > 1){
				...
				executeSearch(currentpage.replace(/_+/g,' '));
	...
	function rebindClick(){
		...
		$('div.a').click(function(event){
			...
			$('#result').animate({opacity: 0},100,function(){
				...
				window.history.pushState(linktext, document.title, linktext.trim().replace(/ +/g,'_'));
				executeSearch(linktext.replace(/_+/g,' '));
