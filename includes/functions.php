...
if(strlen($_SERVER['REQUEST_URI']) > 1){
  ...
  $meta_title = $word.' definition by Wordo';
...
function wordo_search_word($word){
  global $meta_desc;
	...
	$typelist = array("Adjective","Noun","Verb","Adverb","Pronoun","Proper noun","Article","Determiner","Preposition","Interjection","Initialism");
    ...
    if(preg_match_all('/==English==[\s\S]*?----/',$data,$matches) == 0)
    ...
    $meta_desc = trim($meta_desc);
		$meta_desc = strlen($meta_desc)>140?substr($meta_desc,0,137).'...':substr($meta_desc,0,140);
	}
}
...
function wordo_get_meta_title(){
	global $meta_title;
	return $meta_title;
}
function wordo_get_meta_desc(){
	global $meta_desc;
	return $meta_desc;
}
